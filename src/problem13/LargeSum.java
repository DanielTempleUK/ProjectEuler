...
public class LargeSum {
	...
	private static int[][] digits = {{3, 7, 1, 0, 7, 2, 8, 7, 5, 3, 3, 9, 0, 2, 1, 0, 2, 7, 9, 8, 7, 9, 7, 9, 9, 8, 2, 2, 0, 8, 3, 7, 5, 9, 0, 2, 4, 6, 5, 1, 0, 1, 3, 5, 7, 4, 0, 2, 5, 0},
	...
	private static String calculateSolution() {
		...
		final List<Long> sumDigits = new ArrayList<Long>();

		long carryOver = 0;

		for ( int i = 49; i >= 0; i-- ) {
			//Total up all the digits in the base.
			long sum = 0;
			for ( int j = 0; j<100; j ++ ) {
				sum = sum + digits[j][i];
			}

			//Add any carry over from the previous base
			sum = sum + carryOver;

			//Mod 10 to get the individual digit for this base
			sumDigits.add(sum % 10);

			//Work out the carry over to add onto the next total
			carryOver = (int) sum / 10;
		}

		//Make sure any remaining carry over digits are added to the sumDigits
		final String carryOverString = String.valueOf(carryOver);
		for (final char character : carryOverString.toCharArray()) {
			sumDigits.add(Long.valueOf(""+character));
		}

		//Build a string containing the first 10 digits
		final StringBuilder builder = new StringBuilder();
		for( int i = 1; i<11; i++) {
			builder.append(""+ (sumDigits.get(sumDigits.size()-i)) );
		}
